...
import { IGetAllUserService } from '@root/domain';
...
describe('Get All User Service Unit', () => {
  ...
  it('shoud be findUserRepo.all to have been called with empty params', async () => {
    const spy = jest.spyOn(findUserRepoMock, 'all');
    await service.start();
  ...
  it('shoud throw error if findUserRepo.all to return an error ', async () => {
    findUserRepoMock.all.mockRejectedValueOnce(new Error('QUERY_ERROR'));
  ...
  it('shoud be service return array empty if findUserRepo.all not find users ', async () => {
    findUserRepoMock.all.mockResolvedValueOnce([]);
